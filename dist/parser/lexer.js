...
(function (TokenType) {
    ...
    TokenType["ALIAS_COMMAND"] = "ALIAS_COMMAND";
    TokenType["JSON_PAYLOAD"] = "JSON_PAYLOAD";
    ...
    tokenise() {
        ...
        while (this.position < this.input.length) {
            ...
            if (char === "{") {
                ...
                if (depth > 0) {
                    throw new SyntaxError(`Lexer Error on line ${startLine}: Unclosed JSON payload bracket '{'`);
                }
                tokens.push({
            ...
            if (/[a-zA-Z_]/.test(char)) {
                ...
                if (lowerIdent === "feature" || lowerIdent === "scenario") {
                    ...
                    if (this.input[tempPos] === '"') {
                        throw new SyntaxError(`Syntax Error on line ${this.line}: '${ident}' declarations must not use quotation marks.`);
                    }
                    if (this.input[tempPos] !== ":") {
                        throw new SyntaxError(`Syntax Error on line ${this.line}: '${ident}' declaration is missing a colon (:).`);
                    }
                    this.position = tempPos + 1;
                    tokens.push({
                        type: lowerIdent === "feature" ? TokenType.FEATURE : TokenType.SCENARIO,
                        value: ident,
                        line: this.line,
                    });
                    let description = "";
                    ...
                    if (!description.trim()) {
                        throw new SyntaxError(`Syntax Error on line ${this.line}: '${ident}' declaration must have a description.`);
                    }
                    tokens.push({
                        type: TokenType.STRING,
                ...
                if (expectingNakedCommand) {
                    ...
                    if (lowerIdent.startsWith("flow.")) {
                        ...
                        throw new SyntaxError(`Syntax Error on line ${this.line}: Do not repeat 'flow.' after 'flow.try'. Use '${correctWord}' instead.`);
                    }
                    ...
                    if (validNakedCommands.map((c) => c.toLowerCase()).includes(lowerIdent)) {
                        tokens.push({
                        ...
                        throw new SyntaxError(`Syntax Error on line ${this.line}: Expected an action command after 'flow.try', but found "${ident}".`);
                    }
                }
                if (["given", "when", "then", "and"].includes(lowerIdent)) {
                    ...
                    while (tempPos < this.input.length &&
                        ...
                        tempPos++;
                    if (this.input[tempPos] !== '"') {
                        throw new SyntaxError(`Syntax Error on line ${this.line}: Block '${ident}' must be immediately followed by a description in double quotes ("").`);
                    }
                    ...
                    if (lowerIdent === "flow.try")
                        expectingNakedCommand = true;
                }
                else if ([
                    "text",
                    "visible",
            ...
            if (char === "/") {
                ...
                throw new SyntaxError(`Lexer Error on line ${this.line}: Found a stray "/". Only "//" and "/* */" comments are allowed.`);
            }
            throw new SyntaxError(`Lexer Error on line ${this.line}: Unexpected character "${char}"`);
        }
