...
class WebAdapter {
    ...
    async initialise(audioInjectionPath) {
        ...
        if (audioInjectionPath) {
            launchArgs.push(`--use-file-for-fake-audio-capture=${audioInjectionPath}`);
        }
    ...
    async switchTo(target) {
        ...
        if (targetLower === "next") {
            ...
            if (currentIndex < pages.length - 1) {
                ...
                throw new Error(`No new tab or window was detected after waiting ${timeout / 1000} seconds.`);
            }
        }
            ...
            if (!found) {
                throw new Error(`Could not find any open tab matching "${cleanTarget}".`);
            }
        }
        await this.page.bringToFront();
        await this.page.waitForLoadState("domcontentloaded").catch(() => { });
    }
    async injectStorage(cookies, localStorage, fallbackDomain) {
        if (!this.context || !this.page)
            return;
    ...
    async getState() {
        ...
        if (this.page && this.page.url() !== "about:blank") {
            try {
                ...
                console.warn("Warning: Could not access storage. Ensure page is loaded.");
            }
        }
    ...
    synthesizeCurl(method, url, headers, postData) {
    ...
    async checkAmbiguity(loc, selector) {
        if (/\[\d+\]/.test(selector))
            return loc.first();
        ...
        for (let i = 0; i < count; i++) {
            ...
            for (const u of uniqueElements) {
                const isIntersecting = !(box.x > u.box.x + u.box.width ||
            ...
            if (!isDuplicate) {
                ...
                const isActionable = ["a", "button", "input", "select"].includes(tagName) ||
        ...
        throw new Error(`Ambiguity Error: I found ${uniqueElements.length} visually distinct elements matching "${selector}" on the screen. To fix this, please specify an element type (e.g., "link" under "Cars") or use a 0-based index (e.g., "[0]").`);
    }
    async close(targetText) {
        const cleanText = targetText.replace(/^['"]|['"]$/g, "");
        const textLoc = this.page.locator(`:text-is("${cleanText}"), :has-text("${cleanText}")`)
            .filter({ visible: true })
            .last();
        if ((await textLoc.count()) === 0) {
            throw new Error(`Geometric Close Error: Could not find any open modal containing the text "${cleanText}".`);
        }
        const clickCoords = await textLoc.evaluate((node) => {
            ...
            while (container && container !== document.body) {
                ...
                const isModalClass = container.className &&
                    typeof container.className === "string" &&
                    container.className.toLowerCase().includes("modal");
                const isFixedPopup = (style.position === "fixed" || style.position === "absolute") &&
            ...
            if (!container || container === document.body) {
                container = node.parentElement?.parentElement || node;
            }
            const box = container.getBoundingClientRect();
            const candidates = Array.from(container.querySelectorAll('button, svg, a, [role="button"], img'));
            ...
            candidates.forEach((el) => {
                ...
                if (elBox.width === 0 || elBox.height === 0)
                    return;
                const isTopRight = elBox.left >= box.left + box.width * 0.5 &&
                    elBox.top <= box.top + box.height * 0.5;
                if (isTopRight) {
                    const distance = Math.sqrt(Math.pow(topRightX - elBox.right, 2) +
            ...
            if (bestCandidate) {
                const finalBox = bestCandidate.getBoundingClientRect();
                return {
    ...
    async click(selector, elementType) {
    ...
    async type(selector, value, elementType) {
        const loc = await this.getLocator(selector, elementType, false, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        await targetLoc.waitFor({ state: "visible" });
        await targetLoc.scrollIntoViewIfNeeded();
        ...
        if (!success) {
            throw new Error(`Framework rejected typing. Expected "${value}" but the field remained empty or mismatched.`);
        }
    }
    async passcode(selector, value, elementType) {
        ...
        if (count === 0) {
            inputs = this.page.locator('input[maxlength="1"], input[inputmode="numeric"], input[autocomplete="one-time-code"]');
            count = await inputs.count();
            if (count > 8) {
                inputs = this.page.locator('form input[maxlength="1"], [role="dialog"] input[maxlength="1"]');
                count = await inputs.count();
            }
        }
        if (count === 0) {
            throw new Error(`Passcode Error: Could not find any input fields (even hidden ones) matching "${selector}".`);
        }
        if (count === 1) {
            await inputs.fill(value, { force: true });
            await inputs.evaluate((node, val) => {
                node.value = val;
        ...
        for (let i = 0; i < Math.min(chars.length, count); i++) {
            ...
            await inputLoc
                ...
                .catch(() => { });
            await inputLoc.evaluate((node) => (node.value = ""));
            try {
                await inputLoc.pressSequentially(chars[i], { delay: 30 });
            }
            catch {
                await inputLoc.evaluate((node, char) => {
                    node.value = char;
                }, chars[i]);
            }
            await inputLoc.evaluate((node) => {
                node.dispatchEvent(new Event("input", { bubbles: true, composed: true }));
                node.dispatchEvent(new Event("change", { bubbles: true, composed: true }));
            });
    ...
    async check(selector, elementType) {
        const loc = await this.getLocator(selector, elementType, false, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        await targetLoc.waitFor({ state: "visible" });
        await targetLoc.scrollIntoViewIfNeeded();
    ...
    async uncheck(selector, elementType) {
        const loc = await this.getLocator(selector, elementType, false, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        await targetLoc.waitFor({ state: "visible" });
        await targetLoc.scrollIntoViewIfNeeded();
    ...
    async upload(selector, filePath, elementType) {
        const loc = await this.getLocator(selector, elementType, false, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        await targetLoc.waitFor({ state: "visible" });
        await targetLoc.scrollIntoViewIfNeeded();
        const absolutePath = path.resolve(process.cwd(), filePath);
        if (!fs.existsSync(absolutePath)) {
            throw new Error(`File Upload Error: Could not find the file at "${absolutePath}". Please ensure the file exists in your project directory.`);
        }
        try {
            ...
            try {
                ...
                throw new Error(`Failed to upload file. The element "${selector}" did not trigger a native file dialog, nor is it a direct file input.`);
            }
        }
    }
    async set(selector, value, elementType) {
        const loc = await this.getLocator(selector, elementType, true, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        const isDateValue = /^\d{4}-\d{2}-\d{2}/.test(value);
        await targetLoc.evaluate((node, val) => {
    ...
    async fallbackCalendarSelect(targetLoc, dateValue) {
        await targetLoc.click({ force: true });
        ...
        const accessibleCellLocator = this.page.locator(`[role="gridcell"][aria-label*="${dateLabelPart}"][aria-label*="${year}"], ` +
            `[role="button"][aria-label*="${dateLabelPart}"][aria-label*="${year}"]`).filter({ visible: true });
        const rawClassLocator = this.page.locator(`.react-datepicker__day--0${day.toString().padStart(2, "0")}, ` +
            `.day:has-text("${day}")`).filter({ visible: true });
        const targetDay = (await accessibleCellLocator.count()) > 0
        ...
        if ((await targetDay.count()) > 0) {
            ...
            throw new Error(`Framework rejected direct date injection, and semantic calendar scanner could not find a visible cell for "${month} ${day}, ${year}".`);
        }
    }
    async select(option, selector, elementType) {
        ...
        await targetLoc.waitFor({ state: "visible" });
        const isReady = await targetLoc.evaluate((n) => {
        ...
        const activeTagName = await activeElement.evaluate((n) => n?.tagName?.toLowerCase());
        if (activeTagName === "input") {
            await activeElement.asElement()?.fill(option);
            await this.page.waitForTimeout(400);
        }
        const dropdownClasses = '.select__menu, .MuiPopover-root, [role="listbox"], [role="menu"], .dropdown-menu';
        ...
        if (!optionFound) {
            throw new Error(`Could not find the option "${option}" in the dropdown "${selector}".`);
        }
        await optionLoc.scrollIntoViewIfNeeded().catch(() => { });
        await optionLoc.click({ force: true });
        await this.page.waitForTimeout(200);
    }
    async drag(sourceSelector, destinationSelector) {
        const sourceLocRaw = await this.getLocator(sourceSelector, undefined, false, false);
        const sourceLoc = await this.checkAmbiguity(sourceLocRaw, sourceSelector);
        const destLocRaw = await this.getLocator(destinationSelector, undefined, false, false);
        ...
        try {
            ...
            if (srcEl && dstEl) {
                await this.page.evaluate(([s, d]) => {
                    const actualSrc = s.closest('[draggable="true"]') || s;
                    const actualDst = d.closest('[draggable="true"]') || d;
    ...
    async forceClick(selector, elementType) {
        const cleanSelector = selector.replace(/^['"]|['"]$/g, "");
        if (cleanSelector.toLowerCase() === "verify you are human" ||
            cleanSelector.toLowerCase() === "cloudflare") {
            const cfWrapper = this.page.locator('iframe[src*="challenges.cloudflare.com"], iframe[title*="Widget containing a Cloudflare security challenge"]').first();
            await cfWrapper
    ...
    async forceType(selector, value, elementType) {
        const loc = await this.getLocator(selector, elementType, true, false);
        const targetLoc = await this.checkAmbiguity(loc, selector);
        await targetLoc.fill(value, { force: true });
    }
    async expectVisible(selector, elementType) {
        const loc = await this.getLocator(selector, elementType, false, true);
        await loc.first().waitFor({ state: "visible" });
    }
    async expectText(text, fuzzy = false, elementType) {
        const loc = await this.getLocator(text, elementType, false, true, fuzzy);
        await loc.first().waitFor({ state: "visible" });
    }
    async expectValue(selector, val, elementType) {
        const loc = await this.getLocator(selector, elementType, false, true);
        const actual = await loc.first().inputValue();
        if (actual !== val)
            throw new Error(`Expected ${val}, got ${actual}`);
    }
    async expectTranscript(expectedText) {
        ...
        throw new Error(`Transcript Assertion Failed: Expected to see/hear "${expectedText}", but the phrase never appeared in the active layout.`);
    }
    async getAttribute(selector, attr, elementType) {
        const loc = await this.getLocator(selector, elementType, true, true);
        return await loc.first().getAttribute(attr);
    }
    isRawCssSelector(selector) {
        if (/\b(near|above|under|below|leftOf|rightOf|inside|in)\s+['"]/i.test(selector))
            return false;
        if (/^[#\.\[]/.test(selector))
            return true;
        if (/^[a-zA-Z0-9]+\s*[\.\#:][a-zA-Z0-9_\-]+/.test(selector))
            return true;
        if (/^[a-zA-Z0-9]+\s*\[(?!\d+\])/.test(selector))
            return true;
        if (/^(text|css|xpath)=/.test(selector))
            return true;
        const htmlTags = [
            "button",
            "nav",
        ...
        if (htmlTags.includes(selector.toLowerCase()))
            return true;
        return false;
    }
    buildHeuristicSelector(targetText, elementType, allowStaticText = false, fuzzy = false) {
        const cleanText = targetText.replace(/^['"]|['"]$/g, "");
    ...
    async getLocator(rawSelector, elementType, isForceAction = false, isAssertionOrAnchor = false, fuzzy = false) {
        ...
        let relation = "";
        let anchor = "";
        const spatialRegex = /\b(near|above|under|below|leftOf|rightOf|inside|in)\s+(['"].*?['"])/i;
        ...
        let finalSelector = targetIsRaw
            ? targetClean
            : this.buildHeuristicSelector(targetClean, elementType, isAssertionOrAnchor, fuzzy);
        ...
        if (arrayIndex !== null)
            return targetLoc.nth(arrayIndex);
        return targetLoc;
    }
    async clearHUD() {
    ...
    async updateHUD(id, text, status, error) {
        if (this.config.headless || !this.page)
            return;
        await this.page.evaluate(({ id, text, status, error }) => {
            const h = window.FlowstrideHUD;
            if (h)
                h.updateStep(id, text, status, error);
        }, { id, text, status, error });
    }
    async shutdown(videoTargetPath) {
        try {
            ...
            console.warn("[WebAdapter]: Warning during browser shutdown or video transfer.");
        }
        finally {
            if (this.browser) {
